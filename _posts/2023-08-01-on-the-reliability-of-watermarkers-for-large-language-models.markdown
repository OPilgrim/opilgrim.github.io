...
本文又设计了一种 synthetic 但 realistic 的攻击方法：将带水印的文本片段嵌入没有水印的人类编写的文档中。这创建了异构示例，其中只有少数文本的子跨度包含异常高数量的绿色标记，表示水印。攻击方法有两个参数，(1)带水印的跨插入的数量和(2)表示带水印文本的结果文档的比例。例如，考虑一个包含10%水印标记和3个插入的段落。如果原始文本有1000个标记，那么这意味着将在封闭的人类文本块中插入33个标记的3个带水印的跨度。我们将此设置命名为“CP-3-10%”，这是“复制-粘贴，3个跨度，10%水印”的缩写。
...
刚在看一篇文章https://arxiv.org/pdf/2306.04634.pdf，作者是 A Watermark for Large Language Models 的作者。这篇文章证明了，即使有水印的文本被恶意删除水印，只要模型能够浏览足够长的token，就仍然可以识别出微弱的水印
