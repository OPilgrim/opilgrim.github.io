...
第二步是比较事实性场景和反事实场景中的输出。仅输入言论的模型的输出$O_c$可认为存在严重的偏置，因为它只是简单地拟合言论中的词语和标签之间的虚假相关性。为了减少这种偏置，受潜在结果模型（Potential Outcomes Model）的启发，我们从$O_{c,e}$中减去$O_{c}$，得到去除偏置的输出$O_u$，
